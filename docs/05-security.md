...
| `arrivals` | של החבורות שלי | דרך RPC בלבד |
| `exercises`, `shop_items` | הכול | — |

**חברי החבורה רואים את האימונים שלך, את הדירוג שלך ומתי אתה במכון. הם לא רואים את הארנק שלך, את
הרכישות שלך, ולא כמה שילמת לנו.**

נוכחות היא מידע רגיש יותר מאימון שהסתיים — היא אומרת **היכן אתה עכשיו**. לכן היא מוגבלת לאותו גבול
tenant בדיוק, פגה מעצמה, ומוצהרת ולא נמדדת ב-GPS: המערכת לעולם אינה מחזיקה את המיקום שלך, רק את
העובדה שהצהרת שאתה בחדר כושר.
...
| 11 | אין מסך "מחק את החשבון שלי" | בינונית | המחיקה **אפשרית** ברמת המסד (ראו 11.1); חסר רק מסך | פעולת מחיקה עצמית |
...
### 11.1 פגם שנמצא ותוקן: מחיקת חשבון הייתה חסומה

בזמן QA התגלה שמחיקת משתמש נכשלת עם `Database error deleting user` ו-500 — עבור **כל מי שאי פעם
יצר חבורה**. הסיבה: כל מפתח זר שמצביע על `profiles` הוגדר `ON DELETE CASCADE` חוץ מ-
`havuras.created_by`, שנשאר בברירת המחדל `RESTRICT`.

זהו פגם פרטיות ולא רק פגם נתונים: משתמש שמבקש שיימחק לא היה יכול להימחק.

**התיקון הנאיבי היה מסוכן בהרבה.** `CASCADE` היה גורם לכך שמחיקת המייסד תמחק את החבורה, ושרשור
החבורה היה מוחק את **האימונים של כל שאר החברים** — כלומר, אדם אחד שמממש את זכותו למחיקה היה
מוחק את הנתונים של תשעה אנשים אחרים. `SET NULL` (מיגרציה `0009`) הוא המודל הנכון: החבורה שורדת
את החשבון שפתח אותה, והבעלות האמיתית ממילא נשמרת ב-`havura_members`.
